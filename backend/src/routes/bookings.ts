...
bookingsRoutes.post('/', async (c) => {
  ...
  const result = await execute(c.env, `
    INSERT INTO bookings (feest_datum, type_feest, naam_organisator, naam_partner1, naam_partner2, bedrijfsnaam, email, telefoon, adres_organisator, btw_nr, access_token, slug, basisprijs, extra_prijzen, totaalprijs, verjaardag_naam_leeftijd, is_aanvraag, locatie_naam, locatie_adres, speakers_aanwezig, licht_aanwezig, dj_booth_aanwezig, opmerkingen, venue_id)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `, [datum, type, naam, body.naam_partner1 ?? null, body.naam_partner2 ?? null, body.bedrijfsnaam ?? null, body.email || '', body.telefoon || '', body.adres_organisator ?? null, body.btw_nr ?? null, token, finalSlug, body.basisprijs ?? null, body.extra_prijzen ?? null, body.totaalprijs ?? body.basisprijs ?? null, body.verjaardag_naam_leeftijd ?? null, isAanvraag, body.locatie_naam ?? null, body.locatie_adres ?? null, body.speakers_aanwezig ? 1 : 0, body.licht_aanwezig ? 1 : 0, body.dj_booth_aanwezig ? 1 : 0, body.opmerkingen ?? null, venueId])
...
bookingsRoutes.patch('/:id/basisinfo', async (c) => {
  const id = c.req.param('id')
  const body = await c.req.json()
  const feestCategorie = body.feest_categorie as string | undefined
  const geldigeCategorieen = ['Trouw', 'Verjaardagsfeest', 'Jubileumfeest', 'Pensioenfeest', 'Bedrijfsfeest', 'Familiefeest', 'Anders', 'Algemeen feest']
  if (feestCategorie !== undefined && !geldigeCategorieen.includes(feestCategorie)) {
    return c.json({ error: 'Ongeldig feesttype' }, 400)
  }
  if (feestCategorie !== undefined) body.type_feest = feestCategorie === 'Trouw' ? 'Trouw' : 'Algemeen'
  delete body.feest_categorie
  ...
  if (body.opmerkingen !== undefined) { fields.push('opmerkingen = ?'); values.push(body.opmerkingen || null) }
  ...
  if (body.type_feest !== undefined) { contractFields.push('event_type = ?'); contractValues.push(feestCategorie || body.type_feest) }
